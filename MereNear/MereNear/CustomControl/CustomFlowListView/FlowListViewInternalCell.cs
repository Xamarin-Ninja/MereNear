        ...
        async Task ExecuteTapGestureRecognizer(View view)
        {
            ...
            if (flowListView != null)
            {
                ...
                try
                {
                    ...
                    if (Application.Current.Properties.ContainsKey("LastSelectedValue"))
                    {
                        if (Application.Current.Properties["LastSelectedValue"] == view.BindingContext)
                        {
                            return;
                        }
                        else
                        {
                            var lastValue = Application.Current.Properties["LastSelectedValue"];
                            flowListView.FlowPerformTap(view, view.BindingContext, lastValue);
                        }
                    }
                    else
                    {
                        flowListView.FlowPerformTap(view, view.BindingContext, null);
                    }

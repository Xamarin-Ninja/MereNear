...
namespace MereNear.CustomControl
{
    public class SwipeCardView : ContentView
    {
        ...
        private const float BackCardScale = 0.4f; // Back card scale
        ...
        public void InvokeSwipeLeft(int numberOfTouches, int differenceX,int differenceY)
        {
        ...
        /// <param name="numberOfTouches">Number of touch events. It should be positive number (i.e. 50)</param>
        /// <param name="differenceX">Distance betweeen two touches. It should be positive number (i.e. 50)</param>
        public void InvokeSwipeRight(int numberOfTouches, int differenceX, int differenceY)
        {
            if (numberOfTouches <= 0 || differenceX <= 0 || differenceY <= 0)
            {
                return;
        ...
        private void HandleTouch(float differenceX, float differenceY)
        {
            ...
            if (topCard.IsVisible)
            {
                ...
                var rotationXAngle = (float)(CardRotationAdjuster * Math.Min(differenceX / this.Width, 1.0f));
                topCard.Rotation = rotationXAngle * DegreesToRadians;


                var rotationYAngle = (float)(CardRotationAdjuster * Math.Min(differenceY / this.Width, 1.0f));
                topCard.Rotation = rotationYAngle * DegreesToRadians;
                ...
                if (Math.Abs((int)this.cardDistance) > this.CardMoveDistance)
                {
                    if (this.cardDistance > 100)
                    {
                        ...
                        var mylayout = cont.GetType();

                        if (mylayout == typeof(Grid))
                        {
                            Grid fGrid = (Grid)cont;

                            var listChildren = fGrid.Children;
                            ...
                            if (theLabel == typeof(Label))
                            {
                                ...
                                emailLabel.Text = "Accepted";
                                emailLabel.HorizontalOptions = LayoutOptions.Start;
                                emailLabel.VerticalOptions = LayoutOptions.Start;
                                emailLabel.RotateTo(-60);
                        ...
                        #endregion
                        //var sel = this.cards.GetValue(topCardIndex);
                    ...
                    else if(this.cardDistance < -100)
                    {
                        ...
                        if (mylayout == typeof(Grid))
                        {
                            ...
                            if (theLabel == typeof(Label))
                            {
                                ...
                                emailLabel.Text = "Rejected";
                                emailLabel.HorizontalOptions = LayoutOptions.End;
                                emailLabel.VerticalOptions = LayoutOptions.Start;
                                emailLabel.RotateTo(60);
                                emailLabel.FontSize = 30;
                                emailLabel.FontAttributes = FontAttributes.Bold;                 
        ...
        private async void HandleTouchEnd()
        {
            ...
            {
                // move off the screen
                    await topCard.TranslateTo(this.Width, 0, AnimationLength / 2, Easing.SpringOut);
                    topCard.IsVisible = false;
                MessagingCenter.Send("RightSwipe", "CardSwipe");
            ...
            else if (this.cardDistance < -100)
            {
                ...
                await topCard.TranslateTo(-this.Width, 0, AnimationLength / 2, Easing.SpringOut);
                topCard.IsVisible = false;
                MessagingCenter.Send("LeftSwipe", "CardSwipe");
                if (this.SwipedLeftCommand != null && this.cardDistance < 0)
                {
            ...
            else
            {
                ...
                MessagingCenter.Send("NoSwipe", "CardSwipe");
                #region MyData
                var a = (Frame)topCard;
                var cont = a.Content;
